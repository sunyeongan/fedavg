...
for k in range(0,update_iter): # ===================모델 업데이트를 10번함 =====================================
    ...
    client_model_file_name = 'model-update-cli_2_model_'+str(k)+'.pt'

    print(f'===서버에게 글로벌 모델 파일 혹은, fedavg된 모델 파일을 {k}번째 전송 요청함 =====')

    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as c2_recv_g:
        c2_recv_g.connect((HOST, PORT)) # 서버로 connect 요청 보냄
        print('서버와 연결 성공!')
        while True:
            ...
            c2_recv_g.sendall('model_request'.encode('utf-8'))  # 모델 요청 신호 전송

            ack_signal = c2_recv_g.recv(1024).decode('utf-8')

            if ack_signal == 'ack':
                print(ack_signal)

                c2_recv_g.sendall(server_model_file_name.encode('utf-8')) # 서버로 글로벌 파일 이름 전송

                data = c2_recv_g.recv(8192) # 서버에게 파일 내용 전송 받음
                data_transferred = 0

                save_dir = os.path.join(current_model_file_path, 'global_model')
                save_dir = os.path.join(save_dir, server_model_file_name)  # 글로벌 모델 파일 저장 경로 경로
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as c2_send_u:
        c2_send_u.connect((HOST, PORT))  # 서버로 connect 요청 보냄
        print('서버와 연결 성공!')
        print('============================서버로 모델 파라미터 파일 전송 ======================================')
        while True:
            c2_send_u.sendall("model_transfer".encode('utf-8'))  # 모델 전송  신호 전송

            ack_signal2 = c2_send_u.recv(1024).decode('utf-8')

            if ack_signal2 == 'ack':
                print(ack_signal2)

                c2_send_u.sendall(client_model_file_name.encode('utf-8')) # client 2 모델 파일 명 서버에 전송
                ...
                with open (update_cli_model_file_path, 'rb') as f:
                    try:
                        ...
                        while data:
                            data_transferred += c2_send_u.send(data) # data 전송
